...
export default function SuppliersPage() {
  ...
  const fetchData = async (page = 1, pageSize = 20) => {
    setLoading(true);
    try {
      const res = await suppliersAPI.list({ page, limit: pageSize, search });
  ...
  const columns = [
    ...
    { title: 'Status', dataIndex: 'is_active', key: 'status', render: (v) => <Tag color={v ? 'green' : 'red'}>{v ? 'Active' : 'Inactive'}</Tag> },
    {
      ...
      render: (_, record) => (
        <Space>
          {hasPermission('suppliers', 'update') && <Button type="link" icon={<EditOutlined />} onClick={() => navigate(`/suppliers/${record.id}/edit`)} />}
    ...
    <Card>
      <div className="page-header">
        ...
        <Space>
          ...
          {hasPermission('suppliers', 'create') && <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/suppliers/new')}>Add Supplier</Button>}
        </Space>
      </div>
